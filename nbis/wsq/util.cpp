   ...
   const int height)       /* image height                 */
{
   ...
   for(cnt = 0; cnt < MAX_SUBBANDS; cnt++) {
      quant_vals->qbss[cnt] = 0.0f;
      quant_vals->qzbs[cnt] = 0.0f;
   }
...
/************************************************************/
void get_lets(
   float *neww,     /* image pointers for creating subband splits */
   float *old,
   ...
   const int inv)        /* spectral inversion? */
{
   ...
   for(rw_cl = 0; rw_cl < len1; rw_cl++) {
      ...
      if(inv) {
         hipass = neww + rw_cl * pitch;
         lopass = hipass + hlen * stride;
      }
      else {
         lopass = neww + rw_cl * pitch;
