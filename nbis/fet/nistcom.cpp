...
{
    char* hst, * fname, * class_str, clazz, * sex, * pname;
    int ret, hst_sz;
    NISTCOM* nistcom;
    ...
    if ((ret = get_sd_class(ihead->id, 4, &clazz))) {
        freefet(nistcom);
        return(ret);
    }
    sprintf(cbuff, "%c", clazz);
...
int sd9_10_14_ihead_to_nistcom(NISTCOM** onistcom, IHEAD* ihead,
    const int sd_id)
{
    char* hst, * fname, * ncic, clazz, * sex, * pname, * ink_liv;
    char* nptr;
    int ret, hst_sz;
    NISTCOM* nistcom;
    char cbuff[11], id_str[BUFSIZE];
    ...
    */
    ...
    if ((ret = get_sd_class(ihead->id, sd_id, &clazz))) {
        freefet(nistcom);
        return(ret);
    }
    sprintf(cbuff, "%c", clazz);
...
{
    int ret, n, seqnum;
    char clazz, ncic_str[BUFSIZE];

    if (sd_id == 4) {
        if ((n = sscanf(id_str, "%*s %c", &clazz)) <= 0) {
        ...
        if ((ret = get_class_from_ncic_class_string(ncic_str, seqnum, &clazz)))
            return(ret);
    }
